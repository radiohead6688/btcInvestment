...
Strategy::Strategy(const Facts& facts, const Config& config) {
    ...
    //m_c = new Controller(facts.elecProp, facts.entryPrice, facts.quantity, config.tProp, config.pProp, config.cProp, facts.pledgeType, facts.durationInDays, facts.tradeFee, facts.leverage, facts.contractSide, facts.netRefiilTimesLimit);
...
//double Strategy::calculate(double elecFee, double usdtRate) {
    //double elecQty = elecFee / usdtRate;
    //double quantity = elecQty / 
